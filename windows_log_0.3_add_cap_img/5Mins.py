...
def call_post_api(url,data,refno):
    try:
        ...
        folder_path_async = f"{path}\PostAsyncResponce.txt"
        with open(folder_path_async, 'a') as file:
            file.write(
                f" {str(datetime.now())} :  {str(response.text)}    " + '\n' + '----------------------------------------------------------------------' + '\n')
            file.write(
                f" {str(datetime.now())} :  {str(files)}    " + '\n' + '----------------------------------------------------------------------' + '\n')
...
sql_quary_get_vehcle_details = """(SELECT vt.id AS id,
       ...
       '' AS vehicleImageb64,vt.iscaptured
...
# FROM VehicleTransaction vt
# INNER JOIN VehicleTransactionImage vti ON vt.id = vti.VehicleTransactionId
# WHERE (vt.dateOfTransaction BETWEEN DATE_SUB('{0}', INTERVAL {1} SECOND) AND '{0}' AND vt.isCaptured = 0) OR
# (vt.dateOfTransaction BETWEEN  '{0}'  AND date_add('{0}', INTERVAL {2} SECOND) AND vt.isCaptured = 0)
        ...
        for i in RequestVehicle:
            ...
            if data != []:
                for veh_details in data:
                    ...
                    if veh_details['iscaptured'] == True:
                        type = 0
                    else:
                        type = 1

                    veh_details['type'] = type
                    ...
                    with open(folder_path_async, 'a') as file:
                        file.write(
                            f" {str(datetime.now())} :  {str(api_Responce)}    " + '\n' + '----------------------------------------------------------------------' + '\n')
...
folder_path = f"{path}\latestgetprevtransaction.txt"

end_time = datetime.now() + timedelta(days=300)
while datetime.now() < end_time:
    ...
    try:
        try:
            ...
            c = db_update_record("""UPDATE requestvehicle SET isActive  = 0 ,RefStatus = 3,isActive0Time = CURRENT_TIMESTAMP  
                                    WHERE DATE(RefNoDatetIME) <= DATE_SUB(CURDATE(), INTERVAL 3 DAY) and RefStatus = 1 and isActive = 1 ;""")
            # print(c)
        except:
            pass
